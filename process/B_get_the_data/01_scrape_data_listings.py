import csv, datetime, time, gc, sys
...
import process.B_get_the_data.vv as vv  # importing
import process.B_get_the_data.pp_demo_local as pp  # importing
...
PAGES_PER_BOROUGH = 25 #10
MAX_LISTINGS = vv.MAX_LISTINGS
MAX_LISTINGS = 150 #70
if DO_PICKUPS:
    MAX_ITEMS = MAX_LISTINGS * 25 + 100
else:
    MAX_ITEMS = MAX_LISTINGS * 25 + 20
...
class Splasher_spider(scrapy.Spider):
    ...
    def start_requests(self):
        ...
        try:
            pd.read_csv(vv.LISTING_JSON_MODEL_FILE, nrows=5)
            header_on_json = False
        except:
            pass
        try:
            pd.read_csv(vv.LISTING_JSON_META_FILE, nrows=5)
        ...
        if not ONLY_PICKUPS:
            ...
            for borough_name, borough_code in pp.BOROUGHS.items():
                ...
                print(f"max allowed: LISTINGS={MAX_LISTINGS}, ITEMS={MAX_ITEMS}")
                ...
                try:
                    ...
                    with open(vv.AUDIT_FILE, 'w') as convert_file:
                        convert_file.write(json.dumps({}, indent=4, sort_keys=True))
                    with open(vv.AUDIT_FILE) as f:
                        raw_audit = f.read()
                ...
                if SCRAPED_LISTINGS > MAX_LISTINGS:
                    print(f"00 exceeded max scrape: ({SCRAPED_LISTINGS} > {MAX_LISTINGS})")
                    return
                ...
                for page_number in range(max(last_page_was, 0), min(last_page_was + PAGES_PER_BOROUGH, 41)):
                    ...
                    if SCRAPED_LISTINGS > MAX_LISTINGS:
                        print(f"0 exceeded max scrape: ({SCRAPED_LISTINGS} > {MAX_LISTINGS})")
                        return
                    ...
                    listings_url = pp.make_listings_url(listings_url=pp.find_listings_url, borough_code=borough_code, q_maxPrice=q_maxPrice, q_minPrice=q_minPrice, q_ascendingSortType=q_ascendingSortType, insertIndex=insertIndex, q_propertyTypes=q_propertyTypes, q_lookback_optional=q_lookback_optional, q_params_optional=q_params_optional)
                    ...
                    if SCRAPED_LISTINGS > MAX_LISTINGS:
                        print(f"1 exceeded max scrape: ({SCRAPED_LISTINGS} > {MAX_LISTINGS})")
                        return
                    # ??? SCRAPED_LISTINGS += 1
    ...
    def pickup_enriching_gaps(self, missable_filename=vv.LISTING_ENRICHED_FILE):
        ...
        if type(missable_filename) == list:
            df_potentially_missing = pd.read_csv(missable_filename[0], on_bad_lines='warn')
    ...
    def parse_front(self, response, main_url, foo, borough, page_number):
        ...
        if SCRAPED_LISTINGS > MAX_LISTINGS:
            print(f"3 exceeded max listings scrape: ({SCRAPED_LISTINGS} > {MAX_LISTINGS})")
            return
        ...
        for apartment in apartments:
            ...
            if SCRAPED_LISTINGS > MAX_LISTINGS:
                print(f"5 exceeded max listings scrape: ({SCRAPED_LISTINGS} > {MAX_LISTINGS}) {borough_name}")
                continue
            # SCRAPED_LISTINGS += 1
            print(
        ...
        if SCRAPED_LISTINGS > MAX_LISTINGS:
            print(f"7 exceeded max listings scrape: ({SCRAPED_LISTINGS} > {MAX_LISTINGS})")
            return
        if SCRAPED_ITEMS > MAX_ITEMS:
        ...
        if DO_ITEMS:
            for code in all_codes:
                if SCRAPED_LISTINGS > MAX_LISTINGS:
                    print(f"8 exceeded max listings scrape: ({SCRAPED_LISTINGS} > {MAX_LISTINGS}) {borough_name}")
                    return
                try:
                    df_indiv_saved2["version"]
                    df_indiv_saved2 = df_indiv_saved2[~df_indiv_saved2.index.duplicated(keep='last')]
        ...
        if not os.path.isfile(vv.LISTING_ENRICHED_FILE):
            ...
            if pp.special_property_1 in model_property["tags"]:
                ...
                try:
                    ...
                    pd.read_csv(vv.LISTING_JSON_MODEL_BID_TRIAL, nrows=5)
                ...
                try:
                    ...
                    pd.read_csv(vv.LISTING_JSON_MODEL_TRIAL, nrows=5)
                    df_property_app.to_csv(vv.LISTING_JSON_MODEL_FILE, mode='a', encoding="utf-8", index=True, header=header_on_json)
                except:
            ...
            if header_on_json == True:
                try:

                    test = pd.read_csv(vv.LISTING_JSON_MODEL_FILE, nrows=5)
                    test['id'].astype('int')
                    header_on_json = False
                    del test
                except:
